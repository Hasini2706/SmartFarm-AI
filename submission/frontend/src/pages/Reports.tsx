...
export const Reports: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        // Query the analytics router or a history endpoint
        const response = await axios.get('/analytics');
        // Filter recent activities to get only diagnoses or fetch directly if available
        const activities = response.data.recent_activities || [];
        const diagnoses = activities
          .filter((act: any) => act.activity_type === 'diagnosis')
          .map((act: any) => {
            // Parse description "Diagnosed Rice with Rice Blast (Conf: 85%)"
            const parts = act.description.split(' with ');
            const crop = act.description.split('Diagnosed ')[1]?.split(' with')[0] || 'Unknown';
            const disease = parts[1]?.split(' (Conf:')[0] || 'Healthy';
        ...
        setError('Failed to fetch historical diagnostic logs.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDownload = (id: number, crop: string) => {
    // Open report download PDF in a new tab
