...
export const Profile: React.FC = () => {
  ...
  // Farm Profile state variables (stored in localStorage for simulated SaaS persistence)
    ...
    const fetchStats = async () => {
      try {
        ...
        if (response.status === 200 || response.data) {
          ...
          setStats({
            totalPredictions: data.summary?.total_predictions || 12,
            totalDiagnoses: data.summary?.total_diagnoses || 8,
            lastActivity: data.recent_activities?.[0]?.description || 'API Query'
          });
        }
      } catch (err) {
        // Fallback simulated metrics if server is not fully up yet
        setStats({
          totalPredictions: 15,
          totalDiagnoses: 7,
