...
class LLMService:
    @staticmethod
    def get_chat_response(query: str) -> Tuple[str, bool]:
        ...
        if cached:
            try:
                ...
                return data["response"], data["voice_supported"]
            except Exception:
                pass
                
        response_text, voice_supported = LLMService._generate_response(query)
        ...
        return response_text, voice_supported

    @staticmethod
    def _generate_response(query: str) -> Tuple[str, bool]:
        query_lower = query.lower()
        
        # Real Gemini integration if API key is provided
        if settings.GEMINI_API_KEY:
            try:
                ...
                payload = {
                    "contents": [{
                        "parts": [{
                            "text": f"You are a helpful, expert AI agricultural consultant on the SmartFarm AI platform. Help the farmer with their query: {query}. Keep answers highly practical, clean, and concise."
                        }]
                    }]
                }
                ...
                print(f"Error calling Gemini API: {e}. Using fallback rule base.")
                
        # Smart Local Rule-Based Fallback
        # 1. Government Schemes
        if any(w in query_lower for w in ["scheme", "government", "subsidy", "pm-kisan", "pmkisan", "yojana"]):
            return (
                "Here are some critical Indian Government schemes for farmers:\n\n"
        ...
        # 3. Market Prices
            ...
            # Generate random but realistic commodity prices
