...
import { BarChart as BarChartIcon, ScatterChart as ScatterIcon, Sprout, RefreshCw } from 'lucide-react';
...
export const Analytics: React.FC = () => {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      ...
      toast.success('Analytics telemetry synchronized from backend.', 'Data Live');
    } catch (err: any) {
      const msg = 'Failed to load live analytical dataset from backend.';
      setError(msg);
      toast.error(msg, 'Analytics Sync Error');
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  // Derived Scatter plot data: pH vs Yield (t/ha) for different crops
  const phYieldData = useMemo(() => {
    return [
  ...
  // Carbon footprint estimation data (tCO2e per hectare)
  ...
  if (error && !analyticsData) {
    return (
      <EmptyState
        title="Analytics Synchronization Failure"
        description={error}
      />
    );
  }
    ...
    <div className="space-y-8 text-left max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-2xl hover:bg-emerald-500/20 font-bold text-xs self-start sm:self-auto transition-all shadow-sm"
        >
