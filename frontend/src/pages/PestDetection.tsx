...
export const PestDetection: React.FC = () => {
  ...
  const handleUploadSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) {
      const msg = 'Please select or drop a pest image first.';
      setError(msg);
    ...
    try {
      ...
      toast.success(`Identified: ${response.data.pest_name} (${response.data.urgency_level} Threat)`, 'Pest Classification Complete');
    } catch (err: any) {
      const msg = err.response?.data?.detail || 'Inference failure. Check pest image parameters and size.';
      setError(msg);
      toast.error(msg, 'Pest Detection Failed');
    } finally {
      setLoading(false);
    }
  }, [file, toast]);
