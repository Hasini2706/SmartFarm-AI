...
export const Irrigation: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Recommended water: ${response.data.water_needed} L/m² (${response.data.schedule})`, 'Irrigation Schedule Calculated');
    } catch (err: any) {
      const msg = err.response?.data?.detail || 'Inference failed. Check values and verify server.';
      setError(msg);
      toast.error(msg, 'Irrigation Forecast Error');
    } finally {
      setLoading(false);
    }
  }, [weather, soilMoisture, temperature, humidity, cropStage, toast]);
