...
export const Fertilizer: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Recommended: ${response.data.recommended_fertilizer}`, 'Fertilizer Recommendation Ready');
    } catch (err: any) {
      const msg = err.response?.data?.detail || 'Fertilizer simulation error. Verify backend is operational.';
      setError(msg);
      toast.error(msg, 'Fertilizer Calculation Error');
    } finally {
      setLoading(false);
    }
  }, [soilType, crop, N, P, K, toast]);
