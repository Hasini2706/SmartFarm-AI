...
export const DiseaseDetection: React.FC = () => {
  ...
  const handleUploadSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) {
      const msg = 'Please select or drop a leaf image first.';
      setError(msg);
      toast.error(msg, 'No File Selected');
      return;
    }
    ...
    try {
      ...
      toast.success(`Diagnosed ${response.data.crop_name}: ${response.data.disease_name}`, 'Diagnosis Complete');
    } catch (err: any) {
      const msg = err.response?.data?.detail || 'Inference failure. Check image properties and file format.';
      setError(msg);
  ...
  const downloadReport = useCallback(() => {
    ...
    toast.info('Downloading diagnostic audit PDF...', 'Report Download');
