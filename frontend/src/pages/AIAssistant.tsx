...
export const AIAssistant: React.FC = () => {
  ...
  const fetchChatHistory = useCallback(async () => {
    try {
      ...
      // Empty chat state on fetch failure
    }
  }, []);
  ...
  const handleClearHistory = useCallback(async () => {
    try {
      ...
      toast.error('Could not clear chat history.', 'Error');
    }
  }, [toast]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      let response;
      try {
        response = await axios.post('/ai/chat', { prompt: text, message: text }, { timeout: 15000 });
      } catch (firstErr) {
        response = await axios.post('/chat/message', { message: text }, { timeout: 15000 });
      }
      ...
      if (err.response?.status === 429) {
        setError('Rate limit reached. Please wait a moment before sending another query.');
      } else if (err.code === 'ECONNABORTED') {
        setError('Request timed out. Please try asking your question again.');
      } else {
        setError(err.response?.data?.detail || 'Failed to transmit message. Verify server connection.');
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center gap-2">
          {messages.length > 0 && (
            <button
              ...
              className="p-2.5 rounded-xl border dark:border-slate-800 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all"
