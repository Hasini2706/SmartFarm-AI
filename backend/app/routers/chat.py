...
@ai_router.post("/chat", response_model=schemas.ChatMessageOut)
...
def send_chat_message(
    ...
    """
    Send a text message / prompt to the AI farming chatbot and retrieve a response.
    Supports POST /api/ai/chat, POST /api/v1/ai/chat, POST /api/v1/chat/message.
    """
    ...
    # Get response from LLM (Gemini or Fallback)
    ai_reply, _, provider = LLMService.get_chat_response(user_query)
    ...
    return {
        ...
        "message": db_ai_msg.message,
        "provider": provider,
        "audio_path": db_ai_msg.audio_path,
...
async def send_voice_chat(
    ...
    # Get response
    ai_reply, voice_supported, provider = LLMService.get_chat_response(query_text)
...
def clear_chat_history(
    ...
    """
    Clear entire chat history for the logged in farmer.
    """
